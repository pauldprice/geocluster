...
Ada County zones (`data/ada_county_evacuation_zones.geojson`, 542 Genasys zones, props `zone/name/status/color/lat/lon`) load as a second overlay (`adaZonesLayer`), styled subtly when Normal and prominently by status color when not. Live status comes from the keyless Genasys aware API (`pollLiveStatus`, every 60s): it polls one point per Ada zone containing households plus all active fire locations, updates `awareStatusByZone`, restyles zones (`restyleAdaZone`), and drives `#alertBanner` (non-Normal zones; clicking an entry selects the zone and fits bounds). The WFS behind the snapshot is origin-allowlisted so it cannot be called from the browser - see `data/README.md`. Boise County live status comes from the county's own public ArcGIS service (`pollBoiseReadiness`, same 60s cycle): `Readiness` of Ready/Set/Go per zone fills the zone with the county's renderer colors (`READINESS_COLORS`), swaps its label to `ZONE · STATUS`, and feeds the same alert banner via `nonNormalZones`. Active Idaho fires and perimeters come from NIFC WFIGS (`refreshFires`, every 5 min, `firesLayer`/`perimetersLayer`, "Fires" toggle). The same refresh pulls NASA FIRMS VIIRS thermal detections (Esri Living Atlas, keyless; Idaho bbox, `hours_old<=96`) - point data whose `scan`/`track` pixel dimensions (km) are synthesized into `L.rectangle` footprints (`buildSatLayer`, `satLayer`), age-banded via `detectionStyle` to match the WFCA fire-map look. Detections <24h are grouped by proximity (`clusterDetections`, 2 km union-find) and each cluster gets a dashed cyan `convexHull` outline (WFCA "SoF cluster" style; WFCA's actual SoF layer is proprietary). There is no keyless public source of FIRMS footprint *polygons*; the rectangles approximate them. GOES detections come keyless from NOAA HMS (`HMS_URL`, `buildGoesLayer`, light translucent orange ~2 km rectangles, ≤6h, age from `YearDay`+`Time` UTC via `hmsAgeHours`), on their own "GOES" toggle independent of the Fires toggle; note the NASA FIRMS API does NOT serve GOES, and HMS latency is ~1-2h (analyst-QC'd). `#liveStatus` in the footer reports poll freshness.

Data-quality flags: rows sharing identical coordinates are "suspect geocodes" (`computeSuspects` -> `suspectByIndex`, red dashed marker ring), valid-coordinate rows matching no zone get an amber dashed ring, and rows with no coordinates are counted in a legend chip (they are not drawn - coordinate parsing uses `parseFloat`, never `Number`, so blank coords don't coerce to 0,0). All marker icons go through `iconFor(idx, selected)`.
